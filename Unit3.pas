...
f.LoadFromFile('AccessFiles.txt');
 for i := 1 to f.Count - 1 do
     begin
       if f.Strings[i]=DocName then   //сравниваем файл из перечня
        break;                           //с введённым
     end;                                //и находим его номер
  f.Free;
...
r:=GetRight(ComboBox1.Text,FormReg.Edit1.Text);
 case r of                 //под этим номером находиться право на этот файл
 0: begin   //собственник              //каждого пользователя(в его личном файле)
     Memo1.lines.LoadFromFile('files/'+ComboBox1.Text+'.txt');
     ComboBox1.Enabled:=false;
    end;
 1: begin   //хозяин               //каждого пользователя(в его личном файле)
     Memo1.lines.LoadFromFile('files/'+ComboBox1.Text+'.txt');
     ComboBox1.Enabled:=false;
    end;
 2: begin   //пользователь
     Memo1.lines.LoadFromFile('files/'+ComboBox1.Text+'.txt');
     ComboBox1.Enabled:=false;
     Memo1.Enabled:=false;
    end;
 3: Memo1.Text:='У вас нет прав для просмотра этого файла';//непользователь
 end;
end;
...
r:=GetRight(ComboBox1.Text,FormReg.Edit1.Text);
 ...
 else ShowMessage('У вас нет прав');
end;
end;
...
//Теперь надо прописать его существование
...
begin
 ...
 r:=GetRight(ComboBox1.Text,FormReg.Edit1.Text);
  ...
  else ShowMessage('Предъявите права!');
f.Free;
g.Free;
end;
...
case r of
 ...
 else ShowMessage('Вы не имеете права так поступать!!!');
end;
end;
...
begin
Memo1.Text:='';
ComboBox1.Items.LoadFromFile('AccessFiles.txt'); //Только из каталога Files
ComboBox1.Items.Delete(0);
ComboBox1.Text:=ComboBox1.Items[0];
//ComboBoxDir.Items:=надо как-то по-хитрому загрузить
end;
...
f:TStrings;
begin
Memo1.Text:='Здравствуй, дорогой пользователь.';
f:=TStringList.Create();
