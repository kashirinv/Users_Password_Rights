...
procedure TFormAdmin.Button8Click(Sender: TObject); //работает
var
 f:TStrings;
begin
 f:=TStringlist.Create();      //тут я хотел добавить группу в файл групп
 f.LoadFromFile('users/'+ComboBox2.Text+'.txt');
 f.Strings[0]:=ComboBox1.Text;
 f.SaveToFile('users/'+ComboBox2.Text+'.txt'); //и мне это удалось!
...
ComboBox1.Visible:=true;    //здесь я открываю всё для создания группы
label3.Visible:=true;
